...
public:
	...
	void DecideForSpellToCast(ACombatController* &Con);

	void PhysicalAttackPriority(ACombatController* &Con);

	void PerformDefend(ACombatController* &Con);
	void PerformAttack(ACombatController* &Con, int32 TargetID);

	void Perform_HP_Spell(ACombatController* &Con);

	void CastSelfBuff(ACombatController* &Con);
